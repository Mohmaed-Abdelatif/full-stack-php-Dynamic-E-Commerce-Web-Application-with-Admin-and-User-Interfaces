...
if (!isset($_GET['action'])) {
    ...
    // Page level custom scripts for chart
...
<script>
    $('.modalbtn').click(function() {
        $(this).parent().parent().find('.view').text('Seen');
        $(this).parent().parent().removeClass("bg-dark");

        let id = $(this).data('id');
        $.post('fun/edit-message.php', {
            messageId: id
        }, function(data) {
            $('.message-count').text(data);
        })
    });
</script>
